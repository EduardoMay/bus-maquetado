...
labelElement.forEach((element) => {
	element.addEventListener('click', function () {
		if (element.classList.contains('available')) {
			element.classList.toggle('occupied-now');
...
let paxComponent = Vue.extend({
	...
	methods: {
		...
		deletePax: function (pax) {
			document
				.querySelector(`span[data-seat_num='${pax.seat}']`)
				.classList.toggle('occupied-now');
			document
				.querySelector(`span[data-seat_num='${pax.seat}']`)
				.classList.toggle('available');
